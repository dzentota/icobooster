...
//        uint256 tokensSold;
    ...
    function ICOBooster(address _owner)
    {
    ...
    function contribute(address campaignId) external payable returns (bool success) {
        require(campaignExist(campaignId));
        require(validPurchase(campaignId));
        require(campaigns[msg.sender].state == State.Active);
...
//        c.wallet.transfer(weiAmount);
    ...
    function validPurchase(address campaignId) internal view returns (bool) {
